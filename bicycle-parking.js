...
SfBikeParking.initialize = function (){
	...
	$.get( "http://data.sfgov.org/resource/w969-5mn4.json?status=COMPLETE&status_detail=INSTALLED", function( data ) {
		if (!data.length) {
			alert('There was a problem downloading the San Francisco Bicycle Parking dataset.'); 
		} else {
...
SfBikeParking.showMarkers = function (event) {
	...
	//Set bicycle racks distance to center
	...
	for (i=0; i < coords.length; i++) {
		c = coords[i];
		var cLatlng = new google.maps.LatLng(c.coordinates.latitude, c.coordinates.longitude);
		var distanceBtwn = google.maps.geometry.spherical.computeDistanceBetween(cLatlng, map.getCenter());
		if (distanceBtwn > map.circle.radius) {
			...
			var marker = new google.maps.Marker({
				position: cLatlng,
				map: map,
				title: c.location_name + ', ' + c.yr_inst + ', type: ' + c.bike_parking + ', spaces: ' + c.spaces
			});
			if (!map.hasOwnProperty("markers")) {
